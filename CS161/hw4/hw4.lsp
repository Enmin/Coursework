...
;
; from at-least-one-color, we get "or" of all colors into a list
; at-most-one-color means the combination that we take one of the
; color in the list or none of them.
; So we pick one color and negate the rest of the colors into
; a new list in "add-negates", and then
(defun add-negates (aloc)
  (cond ((null (cdr aloc)) nil)
        (t (cons (list (- (car aloc)) (- (cadr aloc))) (add-negates (cons (car aloc) (cdr (cdr aloc))))))
  )
)
(defun at-most-one-color (n c k)
    (cond ((> c k) nil)
          (t (append (add-negates (at-least-one-color n c k)) (at-most-one-color n (+ c 1) k)))
    )
  )
...
;
; get x y, set c to 1, which is the start value of set {1,2...k}.
; use node2var to get the value of colored index and set the clause -x or -y
; recursively call the function from 1 to k.
(defun generate-edge-clauses-helper (e c k)
  (let   ((x (car e))
          (y (cadr e))
         )
         (cond ((> c k) nil)
               (t (cons (list (- (node2var x c k)) (- (node2var y c k))) (generate-edge-clauses-helper e (+ c 1) k)))
         )
    )
)
