    ...
    while(1){
        ...
        while(difftime(end,begin) < period){
            ...
            if(pfd[0].revents & POLLIN){
	            ...
	            memset(big,0,1024);
    	        memset(small,0,20);
    	        int num_read=SSL_read(ssl,big,1024);
    	        if(num_read<0){
...
void check_arg(int argc, char* argv[]){
	...
	if(is_right > 0){
		fprintf(stderr,"Invalid Arugments, correct usage: ./lab4b [--period=#], [--scale=] [--log=filename] [--id=#] [--host=address] [#port]\n");
		exit(1);
	}
    ...
    if(SSL_library_init()<0){
        fprintf(stderr,"Error openssl library");
        exit(1);
    }
    method=SSLv23_client_method();
    if((ctx = SSL_CTX_new(method))==NULL){
        fprintf(stderr,"Erro SSL connect\n");
        exit(1);
    }
    ...
    if(SSL_connect(ssl)!=1){
        fprintf(stderr,"Error SSL Session\n");
        exit(1);
    }
    //print("%s,%d,%d\n",id,sockfd,port);
    memset(buffer,0,40);
