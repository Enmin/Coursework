...
def get_tiny_images(image_paths):
    ...
    for image_path in image_paths:
        ...
        image = resize(image, output_shape=(16, 16), anti_aliasing=True)
        Ir = image.flatten()
        Izm = Ir - np.mean(Ir)
...
def build_vocabulary(image_paths, vocab_size):
    ...
    total_features = []
    for i in range(num_imgs): #progressbar(range(num_imgs), "Loading ...", num_imgs):
        image = imread(image_paths[i])
        if len(image.shape) > 2:
            image = rgb2grey(image)
        image = resize(image, output_shape=image_shape, anti_aliasing=True)
        hog_feature = hog(image, pixels_per_cell=(z,z), cells_per_block=(z,z), feature_vector=True)
        hog_features = hog_feature.reshape(-1, z*z*9)
        total_features += hog_features.tolist()
    total_features = np.array(total_features)
    vocab = KMeans(n_clusters=vocab_size, max_iter=100, random_state=0).fit(total_features).cluster_centers_
...
def get_bags_of_words(image_paths):
    ...
    for i in range(num_imgs): #progressbar(range(num_imgs), "Loading ...", num_imgs):
        ...
        hog_feature = hog(image, pixels_per_cell=(z,z), cells_per_block=(z,z), feature_vector=True)
        hog_features = hog_feature.reshape(-1, z*z*9)
        hist_vec = np.zeros(vocab_size)
        distance = cdist(hog_features, vocab, 'euclidean')
        for j in distance:
            top = np.argsort(j)[0]
            hist_vec[top] += 1
        total_hist_vecs.append(hist_vec)

    return np.array(total_hist_vecs)
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    W = []
    C = []
    svms = {cat: LinearSVC(random_state=42, tol=1e-3, loss='squared_hinge', C=0.01) for cat in categories}
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    for i in range(M):
        k_top = np.argsort(distances[i])[:k]
        counter = {}
        labels_top = [train_labels[j] for j in k_top]
        for label in labels_top:
            counter[label] = counter.get(label, 0) + 1
        prediction = sorted(counter.items(), key=lambda item: item[1], reverse=True)[0][0]
