...
class Model:
    def __init__(self, train_images, train_labels, num_classes, hp, test_images, test_labels):
        ...
        self.W = np.random.rand(self.input_size, num_classes)
    ...
    def forward_pass(self, inputs):
        ...
        logits = logits - np.expand_dims(np.max(logits), axis=0)
        logits = np.exp(logits)
        ax_sum = np.expand_dims(np.sum(logits), axis=0)
        probabilities = logits / ax_sum
    ...
    def loss(probabilities, gt_label):
        ...
        probabilities = [np.finfo(float).eps if i == 0 else i for i in probabilities]
        loss = -1 * np.log(probabilities[gt_label])
        return loss
    ...
    def back_propagation(img, probabilities, gt_label):
        ...
        probabilities = probabilities[0]
        I = img.shape[0]
        gt_label = [1 if i == gt_label else 0 for i in range(len(probabilities))]
        J = len(gt_label)
        gradW = [[img[i]*(probabilities[j] - gt_label[j]) if gt_label[j] == 1 else img[i] * probabilities[j] for j in range(J)] for i in range(I)]
        gradB = [probabilities[j] - gt_label[j] if gt_label[j] == 1 else probabilities[j] for j in range(J)]
        return np.array(gradW), np.array(gradB)
    ...
    def train_nn(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # self.train_svm()
            # accuracy = self.accuracy_svm(self.test_images, self.test_labels)
